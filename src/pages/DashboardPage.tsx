...
const PanelCard: React.FC<{
  ...
  <div
    className={`relative overflow-hidden ${CARD} ${CARD_HOVER} ${className}`}
  >
...
const DashboardPage: React.FC = () => {
  ...
  const tab: DashboardTab = DASHBOARD_TABS.includes(
  ...
  const usedTotal = balTypes.reduce((a, t) => a + (balance[t]?.used || 0), 0);
  const quotaTotal = balTypes.reduce((a, t) => a + (balance[t]?.total || 0), 0);
  ...
  // "Leave by Type" 56 bar chart data - role aware.
  const typeData = isAdmin
    ? (adminStats.leavesByType || []).map((t: any) => ({
        name: (t?._id || "other").replace(/^\w/, (c: string) =>
          c.toUpperCase(),
        ),
        value: t?.count || 0,
      }))
  ...
  const miniStats = isAdmin
    ...
    : [
        ...
        },
        {
          ...
          value: recent.filter((l) => l?.status === "pending").length,
          icon: ClockIcon,
        },
      ];
  ...
  const rosterEmployees: any[] = (
    (employeeList as any)?.data?.employees || []
  ...
  const availabilityOf = (emp: any) => {
    ...
    const reported =
      code && dayWasReported ? attendanceToday.get(code) : undefined;
  ...
  const teamMembers = (
    isAdmin && rosterEmployees.length ? rosterEmployees : teamFromLeaves
  )
              ...
              <div className="grid grid-cols-1 items-start lg:grid-cols-3 gap-5">
                ...
                <div className="space-y-5">
                  ...
                  <PanelCard
                    ...
                    action={
                      <SunIcon className="w-4 h-4 text-gray-400 dark:text-gray-500" />
                    }
                  >
